...
export default {
    ...
    async created() {
        // Prefill data license dari API jika ada id
        const route = useRoute();
        const licenseStore = useLicenseStore();
        const id = route.params.id;
        if (id) {
            const data = await licenseStore.getDetailLicenses(id);
            this.license = data;
            if (data) {
                this.form.title = data.title || '';
                this.form.price = data.price || '';
                this.form.description = data.description || '';
                this.form.feature = data.feature || '';
                this.form.info = data.info || '';
                this.form.license_type = data.license_type || '';
                this.form.penjual = data.penjual || '';
                this.form.whatsapp = data.whatsapp || '';
                this.form.sellerid = data.seller_id || '';
                this.form.seller = data.seller_id || '';
                // Pembayaran
    ...
    methods: {
        ...
        async submitLicense() {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                } else if (!this.imageSrcList[i] && this.license && this.license[`image_url_${i+1}`]) {
                    // Image deleted, send empty string to remove
                    formData.append(`image_url_${i+1}`, '');
                } else if (this.imageSrcList[i] && this.license && this.imageSrcList[i] === this.license[`image_url_${i+1}`]) {
                    // Image unchanged, send original URL
                    formData.append(`image_url_${i+1}`, this.license[`image_url_${i+1}`]);
                }
            }
            try {
                ...
                if (id) {
                    await store.editLicense(id, formData);
                } else {
    ...
    watch: {
        selectedPembayaran(newVal) {
            // Pastikan pembayaran adalah object, bukan string
            if (typeof this.form.pembayaran === 'string') {
                try {
                    this.form.pembayaran = JSON.parse(this.form.pembayaran);
                } catch (e) {
                    this.form.pembayaran = { rekber: 'NO', tokped: 'NO', shopee: 'NO', tiktok: 'NO' };
                }
            }
